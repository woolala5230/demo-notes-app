import { OrderByItemNode } from '../operation-node/order-by-item-node.js';
import { AnyRawBuilder } from '../util/type-utils.js';
import { ParseContext } from './parse-context.js';
import { ReferenceExpression } from './reference-parser.js';
export declare type OrderByDirection = 'asc' | 'desc';
export declare type OrderByExpression<DB, TB extends keyof DB, O> = ReferenceExpression<DB, TB> | (keyof O & string);
export declare type OrderByDirectionExpression = OrderByDirection | AnyRawBuilder;
export declare function parseOrderBy(ctx: ParseContext, orderBy: OrderByExpression<any, any, any>, direction?: OrderByDirectionExpression): OrderByItemNode;
